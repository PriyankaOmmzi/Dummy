...
//	public AudioSource mainSound;
	...
	void Update()
	{
		if (PlayerParameters._instance.myPlayerParameter.membership_no == 7) {
			swapstamina.text = "Swap 12.5% Stamina";
			swaporb.text = "Swap 1 Orb";
			SwapOrb.interactable = true;
			SwapStamina.interactable = true;
			VipHelmet.SetActive (true);

		} else if (PlayerParameters._instance.myPlayerParameter.membership_no == 14) {
			swapstamina.text = "Swap 14.3% Stamina";
			swaporb.text = "Swap 1 Orb";
			SwapOrb.interactable = true;
			SwapStamina.interactable = true;
			VipHelmet.SetActive (true);
			...
			swapstamina.text = "Swap 16.6% Stamina";
			swaporb.text = "Swap 1 Orb";
			SwapOrb.interactable = true;
			SwapStamina.interactable = true;
			VipHelmet.SetActive (true);
		} else {
			swapstamina.text = "Swaping not allowed";
			swaporb.text = "Swaping not allowed";
			SwapOrb.interactable = false;
			SwapStamina.interactable = false;
			VipHelmet.SetActive (false);
		}
...
//		Debug.Log ("PlayerDataParse._instance.playersParam.userId = "+PlayerDataParse._instance.playersParam.userIdNo);
	...
	public void FbLogin()
	{
		FacebookController._instance.FbLogin ((success,msgString) =>{
			if(success)
			{
...
//		loader.SetActive (false);
		LoginScene.SetActive(false);
		if (PlayerParameters._instance.myPlayerParameter.avatar_no == 0 && PlayerDataParse._instance.playersParam.avatarNo == 0) {
			charachterSelection.SetActive (true);
		} else {

			charachterSelection.SetActive (false);
			if (!PlayerPrefs.HasKey ("Tutorial")) {
				if (loadingScene.Instance.tutorialScreen != null)
					loadingScene.Instance.tutorialScreen.SetActive (true);
			} else {
//				if (loadingScene.Instance.tutorialScreen != null)
//					Destroy (loadingScene.Instance.tutorialScreen);
			}
	...
	public void GetStashInStart(Action <bool> callBack)
	{
		StartCoroutine (FetchStashInStart (callBack));
	}

	IEnumerator FetchStashInStart(Action <bool> callBack)
	{
		...
		form_time.AddField ("type", "0");
		WWW www = new WWW (loadingScene.Instance.baseUrl, form_time.data);
		yield return www;
		Debug.Log ("stash text = "+www.text);
		IDictionary stashDict = Json.Deserialize(www.text) as IDictionary;
		...
		{
			IList stashList = (IList)stashDict ["data"];
			for (int i = 0; i < stashList.Count; i++) 
			{
				IDictionary stashListDic = (IDictionary)stashList[i];

				if(stashListDic["item_name"].ToString() == "gold")
				{
					if(stashListDic["count"] != null)
						int.TryParse (stashListDic["count"].ToString (), out PlayerParameters._instance.goldStash);
					PlayerParameters._instance.stashIdsToDelete += stashListDic ["id"].ToString () + ",";

				}
				else if(stashListDic["item_name"].ToString() == "wheat")
				{
					if(stashListDic["count"] != null)
						int.TryParse (stashListDic["count"].ToString (), out PlayerParameters._instance.wheatStash);
					PlayerParameters._instance.stashIdsToDelete += stashListDic ["id"].ToString () + ",";
				}
				else if(stashListDic["item_name"].ToString().Contains("artefact"))
				{
					int artefactNo = int.Parse(stashListDic ["item_name"].ToString ().Substring (stashListDic ["item_name"].ToString ().Length - 1,1));
					if(stashListDic["count"] != null)
						int.TryParse (stashListDic["count"].ToString (), out PlayerParameters._instance.artefactsToDelete[artefactNo]);
					PlayerParameters._instance.stashIdsToDelete += stashListDic ["id"].ToString () + ",";
				}
				else if(stashListDic["item_name"].ToString().Contains("currently_deployed_soldiers"))
				{
					if(stashListDic["count"] != null)
						int.TryParse (stashListDic["count"].ToString (), out PlayerParameters._instance.currentlyDeployedSoldiersStash);
					CardsManager._instance.stashIdsToDelete += stashListDic ["id"].ToString () + ",";
				}

				else if(stashListDic["item_name"].ToString() == "event_points")
				{
					if(stashListDic["count"] != null)
						int.TryParse (stashListDic["count"].ToString (), out ChestData._instance.eventPointsStash);
					ChestData._instance.stashIdsToDelete += stashListDic ["id"].ToString () + ",";
				}
				else if(stashListDic["item_name"].ToString() == "overall_points")
				{
					if(stashListDic["count"] != null)
						int.TryParse (stashListDic["count"].ToString (), out ChestData._instance.overallPointsStash);
					ChestData._instance.stashIdsToDelete += stashListDic ["id"].ToString () + ",";
				}
				if (PlayerParameters._instance.stashIdsToDelete.Length > 0) {
					PlayerParameters._instance.stashIdsToDelete = PlayerParameters._instance.stashIdsToDelete.Remove (PlayerParameters._instance.stashIdsToDelete.Length-1,1);
				}
				if (ChestData._instance.stashIdsToDelete.Length > 0) {
					ChestData._instance.stashIdsToDelete = ChestData._instance.stashIdsToDelete.Remove (ChestData._instance.stashIdsToDelete.Length-1,1);
				}
				if (CardsManager._instance.stashIdsToDelete.Length > 0) {
					CardsManager._instance.stashIdsToDelete = CardsManager._instance.stashIdsToDelete.Remove (CardsManager._instance.stashIdsToDelete.Length-1,1);
				}
			}
		}
		callBack (true);
	...
	public void collectStashItem()
	{
		...
		form_time.AddField ("user_id",   PlayerDataParse._instance.playersParam.userId.ToString()); 
		form_time.AddField ("device_id", SystemInfo.deviceUniqueIdentifier);
		form_time.AddField ("stash", PlayerParameters._instance.stashIdsToDelete);
		WWW www = new WWW (loadingScene.Instance.baseUrl, form_time.data);
		...
		StartCoroutine (empireScene.instance.userTIMEfetching2 (www, isSuccess => 
		{
			if(!isSuccess)
			{
				PlayerParameters._instance.myPlayerParameter.gold += PlayerParameters._instance.goldStash;
				PlayerParameters._instance.myPlayerParameter.wheat += PlayerParameters._instance.wheatStash;
			}
		}));
	}
	...
	{
		ChestData._instance.overallPointsStash = 0;
		ChestData._instance.eventPointsStash = 0;
		...
		Debug.Log("IOS" + PushNotificationsManager.instance.RegistrationID());
		wwwForm.AddField ("device_type" , "IPHONE");
		wwwForm.AddField ("device_registration_id" , PushNotificationsManager.instance.RegistrationID());
		#else
		wwwForm.AddField ("device_type" , "");
		wwwForm.AddField ("device_registration_id" , "");
		#endif
		...
		if (wwwLogin.error == null) {
			...
			if(wwwLogin.text.Contains ("error_msg"))
			{
				...
				PlayerDataParse._instance.playersParam.avatarNo = 0;
...
//				PushNotificationsManager.instance.Init(userParamaters["username"].ToString());
				#endif
				...
				PlayerDataParse._instance.playersParam.avatarNo = PlayerParameters._instance.myPlayerParameter.avatar_no;
				PlayerDataParse._instance.SaveData ();

				PlayerParameters._instance.SavePlayerParameters (playerParamaters , false , shouldProceed => {
					if(shouldProceed)
					{
						if(!string.IsNullOrEmpty(PlayerParameters._instance.stashIdsToDelete))
							collectStashItem();
						loadingScene.Instance.readyTogo = true;
						loader.SetActive (false);
						TakeToHomeScreenFromLogin ();
...
//			string loginText = wwwLogin.text.Replace (@"\","");
			...
			else
			{
				...
				PlayerDataParse._instance.playersParam.fblogin =  0;
				#if !UNITY_EDITOR && UNITY_ANDROID
//				PushNotificationsManager.instance.Init(PlayerDataParse._instance.playersParam.userName);
				#endif
				PlayerDataParse._instance.playersParam.referralCode = userParamaters["my_referral_code"].ToString ();
				IDictionary playerParamaters = (IDictionary)resultDict["player"];
				if (playerParamaters ["avatar_no"] != null) {
					int.TryParse (playerParamaters ["avatar_no"].ToString (), out PlayerDataParse._instance.playersParam.avatarNo);
					int.TryParse (playerParamaters ["avatar_no"].ToString (), out PlayerParameters._instance.myPlayerParameter.avatar_no);
				}
...
//						{
							loader.SetActive (true);
						StartCoroutine (CardsManager._instance.GetPlayerCards ((gotCard,msgString) => {
								if(!gotCard)
								{
...
//				}));
	...
	public void CardProbability(int noOfCardsRequired , ref string cardrarity ,  ref string noOfCards)
	{
		...
		if (www.error == null && !string.IsNullOrEmpty(www.text)) {
			...
			if (www.text.Contains ("error_msg")) {
				...
				for (int i = 0; i < no_of_cards; i++) {
					...
					newcard.cardDescription = newCardData ["description"].ToString ();
					...
					newcard.cardSpriteFromResources = Resources.Load<Sprite> ("Archive/" + newcard.card_name);
